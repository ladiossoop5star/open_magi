...
Start three subtasks using the runtime's subagent or task tool. In Codex, first
prefer custom agents generated by `open-magi setup-codex`; in OpenCode, prefer
the configured subagents. If named subagents are available, use:
...
Codex note: when using `spawn_agent`, pass the configured name as
`agent_type`, for example `agent_type="deliberator-melchior"`.
...
OpenCode adapter note: OpenCode does not provide a per-subtask timeout field.
...
then calls OpenCode `session.abort` after `deliberatorTimeoutMs`.
