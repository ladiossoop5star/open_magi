...
The config path is the absolute `<captured-cwd>/.open-magi-herdr`, which may be below the repository root. Before setting `state.active=true`, creating or updating that file, or asking the user to enter command secrets, determine whether the captured directory is in Git. From that directory run `git rev-parse --show-toplevel`, `git rev-parse --show-prefix`, and `git rev-parse --git-path info/exclude`. Form an anchored repository-relative ignore pattern as `/<show-prefix>.open-magi-herdr`, escaping Git-ignore metacharacters in the prefix as needed. An empty prefix yields `/.open-magi-herdr`; a `packages/app/` prefix yields `/packages/app/.open-magi-herdr`. Append that exact pattern idempotently to the resolved repository-local exclude file, then validate the actual absolute config path with `git check-ignore --no-index --quiet -- <absolute-config-path>`. If validation fails, stop before writing secrets or continuing. Linked worktrees may share repository exclusion state; do not claim or assume a per-worktree exclude file.

Outside a Git repository, skip the exclusion commands, create any new config with owner-only permissions, preserve restrictive permissions on updates, and protect the file manually from copying or disclosure. Then read the absolute config path. If it is missing or invalid, ask the user for the configuration, write it without echoing values, revalidate permissions and exclusion when applicable, and read it back to validate contents. Never infer a launch command from runtime configuration, an agent name, PATH conventions, or an earlier report. Never expose raw commands or report contents in diagnostics or command output.
