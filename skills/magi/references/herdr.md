...
If the file exists, reuse the returned metadata; no second `lstat` or existence check is allowed before asking or parsing. If that metadata does not prove current-user ownership, regular-file type, and valid mode, ask immediately without reading. Otherwise read and parse only that exact file using Launch Configuration below. If format, role, or command is invalid, ask immediately after inspecting only that file; do not search elsewhere or create state/question artifacts.
