...
- [ ] Every structured wait result accounts for `stdout`, `stderr`, or labeled `combinedOutput`, and its `exitCode`, `cliResult`, and `parseError` preserve success or failure evidence without a stdout-only assumption.
