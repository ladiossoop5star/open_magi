...
test("package metadata exposes OpenCode plugin, setup CLI, and injected plugin tests", async () => {
  ...
  await access(new URL("../bin/postinstall.js", import.meta.url), constants.X_OK)
...
test("English README documents install and avoids local-only model warnings", async () => {
  ...
  assert.match(readme, /External Headless Deliberators/)
  assert.match(readme, /open_magi\.json/)
  assert.match(readme, /Do not put this custom block in `opencode\.json`/)
  assert.match(readme, /runner[\s\S]*command/)
  assert.match(readme, /OPEN_MAGI_PROMPT_FILE/)
  assert.match(readme, /OPEN_MAGI_REPORT_FILE/)
...
test("Traditional Chinese README exists for zh-TW users", async () => {
  ...
  assert.match(readme, /deepseek-v4-flash/)
  assert.match(readme, new RegExp(`${chars(0x5916, 0x90e8)} Headless Deliberator`))
  assert.match(readme, /open_magi\.json/)
  assert.match(readme, /OPEN_MAGI_PROMPT_FILE/)
  assert.match(readme, /OPEN_MAGI_REPORT_FILE/)
  assert.match(readme, /\.open_magi\/magi-log/)
...
test("bundled magi skill assets contain the expected contract", async () => {
  ...
  assert.match(contract, /External Command Deliberators/)
  assert.match(contract, /OPEN_MAGI_PROMPT_FILE/)
  assert.match(contract, /OPEN_MAGI_REPORT_FILE/)
  assert.match(contract, /external command runner is handling/)
