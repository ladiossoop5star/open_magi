...
test("bundled magi skill assets contain the expected contract", async () => {
  ...
  assert.match(selectionPreflight, /exact file exists/i)
  assert.match(selectionPreflight, /ownership[\s\S]*mode[\s\S]*read[\s\S]*parse only that exact file/i)
  ...
  assert.match(herdr, /verify.*mode.*before writing command content.*every read.*update/is)
  ...
  for (const packagedSkill of [skill, codexSkill, claudeSkill]) {
    ...
    assert.match(hardGate, /exact\s+file\s+exists[\s\S]*(?:ownership|owner)[\s\S]*mode[\s\S]*(?:parse|read)/i)
  ...
  assert.match(skill, /debug_direction[\s\S]*first round[\s\S]*status_assessment[\s\S]*before execution/)
