...
5. Generate one filename-safe turn ID containing only ASCII letters, digits, dot, underscore, and hyphen, with no slash, backslash, `..`, or leading dot. Before the frozen baseline, create exactly three predeclared absolute `waitResultPath` values, one per sage and adjacent to that sage's assigned report: `<absolute-turn-dir>/wait-result-<sage>-<filename-safe-turn-id>.json`. These are exactly three final `waitResultPath` values; freeze all three concrete paths. If the existing atomic writer uses worktree-visible companion files, predeclare and freeze exactly three companion absolute paths, one per role, if used; otherwise zero companion paths are permitted. A directory, glob, category, path discovered after submit, or undefined convention is not an allowed path.
...
Exercise the missing-file branch by explicitly asking to start Magi for this project with `HERDR_ENV=1`. From the already-current working directory, derive the exact absolute `./.open-magi-herdr` path without searching. The exactly one initial filesystem operation is `lstat` of that path. Its single result must determine `ENOENT` versus existing and supply owner, type, and mode metadata. Make it return `ENOENT`; the next and only action must be the first user-facing action, directly asking for the exact `melchior`, `balthasar`, and `casper` commands. Record a filesystem/tool audit proving exactly one initial filesystem operation—the `lstat` of the exact config path—followed by the direct question. The existing branch must reuse the returned metadata, with no second lstat or existence check before asking or parsing. The audit must prove no Herdr call; no repository/project read or search; no Git, chmod, mkdir, reference load, `.open_magi` inspection, state or checklist creation, pane split, or agent launch; `state.active` was not set; and `question-request.md` was not created and remains absent.

Supply the three test-owned commands, then exercise the authorized safe creation path. Apply owner-only `0600` mode where supported, install the exclusion, write atomically without echoing commands, and revalidate the exact file. Only after the file parses and validates locally, call `herdr pane current --current` and capture the source pane and creation working directory before any split. Use that captured creation working directory as the absolute config base even when it is a nested repository directory. From that captured directory, obtain `git rev-parse --show-toplevel`, `git rev-parse --show-prefix`, and `git rev-parse --git-path info/exclude`; calculate the anchored repository-relative exclude pattern from the captured prefix, update the resolved repository exclude idempotently, and validate the absolute config path with `git check-ignore --no-index --quiet`. Stop before launching if the check fails. Verify ownership, regular-file identity, and mode before every later read or update; never read an insecure or unowned config first.

Also exercise a pre-existing invalid exact file in a separate activation. Its single initial `lstat` supplies all owner/type/mode metadata; reuse that result and do not issue a second probe before reading or asking. For a symlink, non-regular entry, foreign-owned entry, or unprovable owner, do not read or mutate it; the direct correction question must be the next and only action, with zero other action. For invalid ownership, type, permissions, format, role, or command, ask that question immediately and prove there was no search elsewhere and no state or question artifact.

For a targeted raw-command correction, the affected role's exact command may appear only in the transient, owner-only `0600` `.open_magi/magi-log/question-request.md`, and that request must set `sensitive: herdr_raw_command`. It must contain no secrets, tokens, or credentials. The plugin consumes and removes that request whether the question is allowed or denied. Make no persistent copy in any report, log, state, history, or wait result; retain only the required digest evidence. On the allowed branch, display the exact command only from this transient request to the operator, then confirm the request was consumed and removed. Existing exact-command digests for configuration drift remain unchanged and distinct from the denial digest below.

Exercise a denied typo or missing-classification branch. Build the test `question:` value from non-command prose, warning text, and a harmless unique sentinel in the test-owned raw command so that hashing only the command cannot pass. The resulting `question-denied` record must contain `sensitive: herdr_raw_command` and `question_sha256`, the sensitive-question digest: lowercase hexadecimal SHA-256 over the exact full `question:` value as parsed, including its prose, warning, and command. Every free-text field must be redacted, the raw-command sentinel must be absent, and the record must contain no secrets, tokens, or credentials. Prove that configuration, controller state, panes, and agents remain unchanged.
...
Launch the three concurrent prompt invocations through runtime process handles. Retain each process handle separately and capture separate stdout and stderr streams when available, or explicitly labeled `combinedOutput` when the runtime exposes only one channel. Do not require or write a wait-result file while its process is running. After each invocation returns, fails, or times out, atomically write that role's frozen `waitResultPath` using the existing controller-write primitive; do not add a runner.

The structured object has exactly these 13 fields in order: `schemaVersion`, `sage`, `agent`, `turnID`, `submittedAt`, `completedAt`, `exitCode`, `timedOut`, `stdout`, `stderr`, `combinedOutput`, `cliResult`, `parseError`. Require `schemaVersion` to be `1`; identity fields must match the role and turn; timestamps must be ISO-8601; `exitCode` is an integer or null; `timedOut` is boolean; stream fields are strings or null; `cliResult` is an object or null; and `parseError` is a string or null. Unavailable streams are null, not omitted.

Parse CLI result JSON from stdout on success, from stderr on a nonzero failure including exit 1, or from `combinedOutput` when only that channel exists. Preserve a valid JSON error object in `cliResult` even on exit 1. On parse failure retain every captured stream and set `parseError`; never replace failure evidence with an empty result. Include a harmless failure fixture that exits 1 with a valid JSON error on stderr, and assert the error survives in `cliResult` with the expected digest. Validate the result JSON schema and digest, then inspect that role's `herdr agent get` result and assigned report together before accepting completion.

No raw prompt, configured command, secret, token, credential, or report body may appear in the structured result. Do not copy report content into a wait-result object.
