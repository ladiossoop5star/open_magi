...
For a targeted raw-command correction, the affected role's exact command may appear only in the transient, owner-only `0600` `.open_magi/magi-log/question-request.md`, and that request must set `sensitive: herdr_raw_command`. It must contain no secrets, tokens, or credentials. The plugin consumes and removes that request whether the question is allowed or denied. Make no persistent copy in any report, log, state, history, or wait result; every other artifact uses only the command's lowercase hexadecimal SHA-256 digest. On the allowed branch, display the exact command only from this transient request to the operator, then confirm the request was consumed and removed.

Exercise a denied typo or missing-classification branch with a harmless unique sentinel in the test-owned raw command. The resulting `question-denied` record must contain `sensitive: herdr_raw_command` and the correct lowercase hexadecimal SHA-256 digest of the exact command. Every free-text field must be redacted, the sentinel must be absent, and the record must contain no secrets, tokens, or credentials. Prove that configuration, controller state, panes, and agents remain unchanged.
