...
Also exercise a pre-existing invalid exact file in a separate activation. Its single initial `lstat` supplies all owner/type/mode metadata; reuse that result and do not issue a second probe before reading or asking. For invalid permissions, type, format, role, or command, ask the direct correction question immediately and prove there was no search elsewhere and no state or question artifact.
