...
Exercise the missing-file branch by explicitly asking to start Magi for this project with `HERDR_ENV=1`. From the already-current working directory, derive the exact absolute `./.open-magi-herdr` path without searching and perform its existence/lstat check. Make that check return `ENOENT`. The next and only action must be the first user-facing action: directly ask for the exact `melchior`, `balthasar`, and `casper` commands. Record a filesystem/tool audit proving exactly one file-existence check of the exact config path followed by the direct question. The audit must prove no Herdr call; no repository/project read or search; no Git, chmod, mkdir, reference load, `.open_magi` inspection, state or checklist creation, pane split, or agent launch; `state.active` was not set; and `question-request.md` was not created and remains absent.
...
Also exercise a pre-existing invalid exact file. Before any Herdr call or other discovery, lstat and read only that exact file; for invalid permissions, format, role, or command, ask the direct correction question immediately and prove there was no search elsewhere and no state or question artifact.
