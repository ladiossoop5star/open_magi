...
然後啟動 OpenCode，直接在 OpenCode 裡設定 Magi：
...
安裝後啟動 OpenCode，執行 `/magi setup`。Magi 應該詢問三個 deliberator 要用的
OpenCode model，寫入 ~/.config/opencode/opencode.json，然後要求重啟 OpenCode。
```

請使用你的 OpenCode `opencode.json` 裡已設定的模型。三個 deliberator 可以共用
同一個 model，也可以讓 Melchior、Balthasar、Casper 各自使用不同 model。如果
還沒 setup 就開始 Magi goal，而且任何 deliberator 還是 `default-model`，Magi
必須停止 goal 並先進入 setup。
...
更新後請重啟 OpenCode。如果 deliberator config 缺失，或仍使用 `default-model`，
請在 OpenCode 內再次執行 `/magi setup`。

## Setup 會寫入哪些檔案

`/magi setup` 會寫入 OpenCode config 目錄，預設位置是：
...
setup 寫入 `opencode.json` 後，請重啟 OpenCode 讓新的 subagent 設定生效。

## Optional CLI fallback

CLI 仍保留給自動化或修復用。不帶 model 參數時，它會寫入 `default-model`
placeholder，讓 `/magi setup` 可以在 OpenCode 內完成設定：

```bash
open-magi setup
```

三個 deliberator 共用已設定好的同一個 model：
