...
setup 時請依提示輸入三個 deliberator 要使用的 OpenCode model。安裝後請確認
~/.config/opencode/opencode.json 裡有 plugin entry，以及三個唯讀 subagent：
deliberator-melchior、deliberator-balthasar、deliberator-casper。也請確認
~/.config/opencode/skills/magi/SKILL.md 存在。setup 也會印出 Magi 設定檔路徑：
~/.config/opencode/magi.json。
```

請使用 OpenCode 可用的模型。三個 deliberator 可以共用同一個 model，也可以讓
Melchior、Balthasar、Casper 各自使用不同 model。使用者可編輯的 Magi 設定放在
`magi.json`；手動修改後重新跑 `open-magi setup`，它會重新產生 `opencode.json`
需要的 runtime 設定。
...
請依提示重新輸入 deliberator models，或使用下面的非互動參數。
...
`magi.json` 是 OpenCode adapter 的唯一使用者可編輯 Magi setup 設定檔，內容保存
plugin spec 和三個 deliberator models：
...
如果 `magi.json` 已存在，`open-magi setup` 可以不帶 model 參數，直接依
`magi.json` 重新產生 OpenCode config。如果 `magi.json` 被刪掉，下次 setup 會
重新詢問模型，除非你提供 flags 或環境變數。
